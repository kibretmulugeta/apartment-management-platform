...
export default function LandlordMessagesPage() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, sender: 'Alex Morgan', content: 'Hi, I submitted a plumbing maintenance ticket regarding the kitchen sink.', time: '10:14 AM', isMe: false },
    { id: 2, sender: 'You', content: 'Thanks Alex! I assigned technician Marcus Vance to inspect the faucet seal.', time: '10:20 AM', isMe: true }
  ]);
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      <main className="flex-1 p-8 flex flex-col h-screen">
        <h1 className="text-2xl font-extrabold text-white mb-6">Tenant & Operations Messaging</h1>
        ...
        <div className="flex-1 glass-panel rounded-2xl border border-slate-800 flex flex-col overflow-hidden">
          <div className="p-4 bg-slate-900/80 border-b border-slate-800 flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-brand-500/20 text-brand-400 flex items-center justify-center font-bold text-xs">AM</div>
            <div>
              <h4 className="text-sm font-bold text-white">Alex Morgan</h4>
              <span className="text-xs text-slate-400">Unit 14B Resident</span>
            </div>
          </div>
